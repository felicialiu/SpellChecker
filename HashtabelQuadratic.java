...
import java.io.*;
/*
 * This class implements the datastructure of a hash table using quadratic
 * probing. A seperate class named HashEntry is used in the Array that represents the
 * hash table.
 */
public class HashtabelQuadratic {
	...
	// Constructor that creates the HashtabelQuadratic object
	public HashtabelQuadratic(int size, Compressable hashfunc) {
		hash_size = size;
	...
	public HashtabelQuadratic(int size, Compressable hashfunc, double factor) {
		hash_size = size;
		hashArray = new HashEntry[hash_size];
		function = hashfunc;
		load_factor = factor;
	...
	public void put(String key, String value) {
		...
		// If slot is occupied, find nextmost free slot using quadratic probing
		} else {
			while(hashArray[index] != null && 
				  hashArray[index].getKey() != key) {
	...
	public void resize(){
		...
		HashtabelQuadratic hashArrayTemp = 
			new HashtabelQuadratic(hash_size, function);
		for (int i = 0; i < hash_size/2; i++) {
			if(hashArray[i] != null){
				hashArrayTemp.put(hashArray[i].getKey(),
