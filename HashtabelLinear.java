...
public class HashtabelLinear {
	...
	// Constructor that creates the HashtabelLinear object
	public HashtabelLinear(int size, Compressable hashfunc) {
		hash_size = size;
	...
	public HashtabelLinear(int size, Compressable hashfunc, double factor) {
		hash_size = size;
		hashArray = new HashEntry[hash_size];
		function = hashfunc;
		load_factor = factor;
	...
	public void resize(){
		...
		HashtabelLinear hashArrayTemp = 
			new HashtabelLinear(hash_size, function);
